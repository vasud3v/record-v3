...
func New(c *cli.Context) (*entity.Config, error) {
	return &entity.Config{
		...
		GoFileEnabled:          c.Bool("gofile-enabled"),
		GoFileAPIToken:         c.String("gofile-api-token"),
		GoFileFolderID:         c.String("gofile-folder-id"),
		GoFileDeleteAfterUpload: c.Bool("gofile-delete-after-upload"),
		SupabaseEnabled:   c.Bool("supabase-enabled"),
		SupabaseURL:       c.String("supabase-url"),
		SupabaseAPIKey:    c.String("supabase-api-key"),
		SupabaseTableName: c.String("supabase-table-name"),
	}, nil
}
