...
type settings struct {
	...
	GoFileEnabled          bool   `json:"gofile_enabled,omitempty"`
	GoFileAPIToken         string `json:"gofile_api_token,omitempty"`
	GoFileFolderID         string `json:"gofile_folder_id,omitempty"`
	GoFileDeleteAfterUpload bool   `json:"gofile_delete_after_upload,omitempty"`

	// Supabase settings
	SupabaseEnabled   bool   `json:"supabase_enabled,omitempty"`
	SupabaseURL       string `json:"supabase_url,omitempty"`
	SupabaseAPIKey    string `json:"supabase_api_key,omitempty"`
	SupabaseTableName string `json:"supabase_table_name,omitempty"`
}
...
func SaveSettings() error {
	s := settings{
		...
		GoFileEnabled:          server.Config.GoFileEnabled,
		GoFileAPIToken:         server.Config.GoFileAPIToken,
		GoFileFolderID:         server.Config.GoFileFolderID,
		GoFileDeleteAfterUpload: server.Config.GoFileDeleteAfterUpload,
		SupabaseEnabled:   server.Config.SupabaseEnabled,
		SupabaseURL:       server.Config.SupabaseURL,
		SupabaseAPIKey:    server.Config.SupabaseAPIKey,
		SupabaseTableName: server.Config.SupabaseTableName,
	}
...
func LoadSettings() error {
	...
	server.Config.GoFileEnabled = s.GoFileEnabled
	server.Config.GoFileAPIToken = s.GoFileAPIToken
	server.Config.GoFileFolderID = s.GoFileFolderID
	server.Config.GoFileDeleteAfterUpload = s.GoFileDeleteAfterUpload
	server.Config.SupabaseEnabled = s.SupabaseEnabled
	server.Config.SupabaseURL = s.SupabaseURL
	server.Config.SupabaseAPIKey = s.SupabaseAPIKey
	if s.SupabaseTableName == "" {
		server.Config.SupabaseTableName = "recordings"
	} else {
		server.Config.SupabaseTableName = s.SupabaseTableName
	}
